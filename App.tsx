...
const App: React.FC = () => {
  ...
  const [view, setView] = useState<
    ...
    | "brand-kit"
  >("landing");
  const [landingLanguage, setLandingLanguage] = useState<"RO" | "EN" | "RU">(
    "RO"
  );
  ...
  const [setConfigs, setSetConfigs] = useState<
    Record<string, GenerationConfig>
  >({});
  ...
  const [lastViewBeforeBrand, setLastViewBeforeBrand] = useState<
    | "landing"
    | "workspace"
    | "gallery"
    | "examples"
  ...
  // Logic State
  const [nextItemCountdown, setNextItemCountdown] = useState<number | null>(
    null
  );
  ...
  const [activeTab, setActiveTab] = useState<"workspace" | "assets">(
    "workspace"
  );
  ...
  useEffect(() => {
    setConfigsRef.current = setConfigs;
  }, [setConfigs]);
  useEffect(() => {
    sheetRefinementRef.current = sheetRefinement;
  }, [sheetRefinement]);
  useEffect(() => {
  ...
  useEffect(() => {
    setProjects((prev) =>
      prev.map((p) => (p.id === activeProjectId ? { ...p, config } : p))
    );
  ...
  const getBackoffDuration = (errorMsg: string, attempt: number) => {
    ...
    const isInternalError =
      errorMsg.includes("500") || errorMsg.includes("Internal Server Error");
  ...
  const handleCreateTypedProject = (payload: {
    name: string;
    icon: string;
    template: any;
    notes?: string;
  ...
  const handleSetSettingsSave = (
    setName: string,
    newConfig: GenerationConfig
  ) => {
  ...
  const processQueue = async (initialQueue: QueueItem[]) => {
    const pendingItems = initialQueue.filter(
    ...
    const finalizeSet = async (
      ...
      await sendTelegramMessage(`📊 SET COMPLETED: ${name}`).catch(() =>
        addLog("Telegram Set Confirm Failed", "error")
      );
    };
    ...
    for (let i = 0; i < pendingItems.length; i++) {
      ...
      if (item.setName !== currentSet) {
        ...
        if (!proj) {
          addLog(
            `Project data missing for item ${item.id}. Skipping.`,
            "error"
          );
          setQueue((prev) =>
            prev.map((q) =>
              q.id === item.id
                ? { ...q, status: "error", error: "Project not found" }
                : q
            )
          );
          continue;
        ...
        if (
          activeSetConfig.uploadedReference &&
          activeSetConfig.uploadedReference === lastUploadedReference &&
          lastSheetUrl
        ) {
        ...
        await sendTelegramMessage(`🎬 STARTING SET: ${currentSet}`).catch(() =>
          addLog("Telegram Start Msg Failed", "telegram")
        );
          ...
          while (
            ...
            try {
              ...
              await sendTelegramDocument(
                sheet,
                `REF_SHEET.png`,
                `🔐 DNA LOCKED: ${currentSet}`
              ).catch(() => addLog("Telegram Sheet Upload Failed", "telegram"));
              await waitWithCountdown(
                STRICT_DELAY_MS,
                "Cooldown after DNA Extraction"
              );
            } catch (e: any) {
              if (!isProcessingRef.current) break; // IMMEDIATE EXIT IF STOPPED
              const errMsg = getErrorDetails(e);
              addLog(
                `Sheet Gen Failed (Attempt ${attempts}): ${errMsg}`,
                "error"
              );
              ...
              if (delay > 15000)
                addLog(
                  `⚠️ High Load Detected. Backing off for ${delay / 1000}s...`,
                  "warning"
                );
      ...
      if (setFailed) {
        setQueue((prev) =>
          prev.map((q) =>
            q.id === item.id
              ? { ...q, status: "aborted", error: "Set Aborted" }
              : q
          )
        );
        continue;
      ...
      let attempts = 0;
      const isFirstSlideOfSet =
        item.slideNumber === "1" ||
        item.slideNumber === "Scene 1" ||
        currentSetItems.length === 1;
      ...
      while (
        ...
        if (attempts > 1)
          setQueue((prev) =>
            prev.map((q) =>
              q.id === item.id
                ? { ...q, status: "retry", attemptCount: attempts }
                : q
            )
          );
        ...
        try {
          ...
          addLog(
            `Generating Slide ${item.slideNumber}: ${item.shortTopic}`,
            "info"
          );
            ...
            prev.map((q) =>
              q.id === item.id
                ? {
                    ...
                    status: "success",
                    imageUrl: base64,
                    referenceImageUsed: currentSheetUrl,
          ...
          currentSetItems = currentSetItems.map((it) =>
            it.id === item.id ? finalItemState : it
          );
          ...
          await sendTelegramDocument(
            base64,
            `img.png`,
            `✅ ${item.setName} - ${item.shortTopic}`
          ).catch(() => addLog("Telegram Slide Upload Failed", "telegram"));
          ...
          if (
            isFirstSlideOfSet &&
            !currentSheetUrl &&
            !execConfig.uploadedReference &&
            !item.sketchReference
          ) {
            addLog(
              "First slide created. Analyzing for DNA Extraction (Slide 1 -> 1.5)...",
              "info"
            );
            ...
            while (
              ...
              try {
                ...
                addLog(
                  `${
                    isPitch ? "Brand" : "Style"
                  } DNA Extracted Successfully (1.5)`,
                  "success"
                );
                await sendTelegramDocument(
                  sheet,
                  `SHEET.png`,
                  `📋 STYLE DNA EXTRACTED: ${currentSet}`
                ).catch(() =>
                  addLog("Telegram Sheet Upload Failed", "telegram")
                );
                await waitWithCountdown(
                  STRICT_DELAY_MS,
                  "Cooldown after DNA Extraction"
                ...
                const errMsg = getErrorDetails(e);
                addLog(
                  `DNA Extraction Failed (Attempt ${sheetAttempts}): ${errMsg}`,
                  "error"
                );
                const delay = getBackoffDuration(errMsg, sheetAttempts);
                if (delay > 15000)
                  addLog(
                    `⚠️ High Load. Backing off for ${delay / 1000}s`,
                    "warning"
                  );
                await waitWithCountdown(delay, "Retry Delay");
              }
          ...
          if (delay > 15000)
            addLog(
              `⚠️ Model Overloaded/Error. Extended cooldown: ${delay / 1000}s`,
              "warning"
            );
    ...
    if (currentSet && isProcessingRef.current && !setFailed)
      await finalizeSet(
        currentSet,
        currentSetItems,
        currentSheetUrl,
  ...
  const handleSaveEditedImage = (newImage: string) => {
    ...
    setQueue((prev) =>
      prev.map((q) =>
        q.id === viewerImage.id ? { ...q, imageUrl: newImage } : q
      )
    );
    ...
    setCompletedSets((prev) =>
      prev.map((set) => ({
        ...
        items: set.items.map((item) =>
          item.id === viewerImage.id ? { ...item, imageUrl: newImage } : item
        ),
      }))
    );
  ...
  const projectCompletedSets = completedSets.filter(
  ...
  projectQueue.forEach((item) => {
    if (!queueBySet[item.setName]) queueBySet[item.setName] = [];
    queueBySet[item.setName].push(item);
  });
  ...
  const projectAssets = [
    ...(config.uploadedReference
      ? [
          {
            type: "User Reference",
            url: config.uploadedReference,
            date: Date.now(),
          },
        ]
      : []),
    ...projectCompletedSets
      .filter((s) => s.sheetUrl)
      .map((s) => ({
        type: `Sheet: ${s.name}`,
        url: s.sheetUrl!,
  ...
  if (view === "project-hub") {
    return (
      <ProjectHub
        ...
        onOpenBrandKit={() => {
          setLastViewBeforeBrand("project-hub");
          setView("brand-kit");
        }}
        onSelectProject={(id) => {
          setActiveProjectId(id);
  ...
  return (
    <div className="flex min-h-screen bg-pampas">
      <div className="fixed top-[75px] right-6 z-[60] flex gap-2">
        <Button
          variant="secondary"
          onClick={() => setView("project-hub")}
          className="shadow-lg"
        >
          Project Hub
        </Button>
        <Button
          onClick={() => {
            setLastViewBeforeBrand(
              view === "project-hub" ? "project-hub" : "workspace"
            );
            setView("brand-kit");
          }}
          className="shadow-lg"
        ...
        >
          ...
          {editingSet && (
            <SetSettingsModal
              ...
              setName={editingSet}
              currentConfig={setConfigs[editingSet] || config}
              onSave={handleSetSettingsSave}
            />
          )}
          <CompletedSetsGallery
            isOpen={!!viewingSet}
            ...
            <>
              ...
              <div className="mb-6 border-b border-ivory-dark flex gap-6 items-center">
                <button
                  ...
                  className={`pb-3 font-serif text-lg font-semibold transition-colors ${
                    activeTab === "workspace"
                      ? "text-crail-500 border-b-2 border-crail-500"
                      : "text-slate-400 hover:text-slate-dark"
                  }`}
                >
                  Workspace
                </button>
              ...
              {activeTab === "assets" ? (
                <div className="grid grid-cols-4 gap-6 animate-fadeIn">
                  {projectAssets.length === 0 && (
                    <p className="text-slate-400 italic col-span-4">
                  ...
                  {projectAssets.map((asset, i) => (
                    <Card key={i} className="p-3">
                      <div
                        className="aspect-square rounded bg-slate-100 mb-2 overflow-hidden cursor-pointer"
                        onClick={() =>
                          setViewerImage({
                            src: asset.url,
                            id: `asset-${i}`,
                      ...
                      <button
                        onClick={() =>
                          handleDownload(asset.url, `asset_${i}.png`)
                        }
                        className="mt-2 text-xs text-crail-500 hover:underline w-full text-left"
                      >
                        Download
                      </button>
                    </Card>
                  ...
                  <div className="w-full lg:w-1/3 flex flex-col gap-6">
                    <div>
                      ...
                      <p className="text-slate-medium text-sm">
                        Zero Tolerance • High Consistency
                      </p>
                    </div>
                    ...
                    <Card elevated className="bg-white">
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <span className="text-xs bg-crail-50 text-crail-600 px-2 py-1 rounded border border-crail-200 font-bold">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <Select
                            ...
                            onChange={(e) =>
                              setConfig({
                                ...config,
                                format: e.target.value as AspectRatio,
                              })
                            }
                            disabled={status !== "idle"}
                          />
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <Select
                            ...
                            onChange={(e) =>
                              setConfig({
                                ...config,
                                textDensity: e.target.value as TextDensity,
                              })
                            }
                            disabled={status !== "idle"}
                          />
                        ...
                        <Select
                          ...
                          onChange={(e) =>
                            setConfig({
                              ...config,
                              searchDepth: e.target.value as SearchDepth,
                            })
                          }
                          disabled={status !== "idle"}
                        />
                        ...
                        <div className="border-t border-ivory-dark pt-4">
                          <label className="block font-serif text-sm font-bold text-slate-dark mb-2">
                            {activeProject?.moduleType === "pitch"
                              ? "Brand Logo Upload (DNA)"
                              : "Character Upload (Global DNA)"}
                          </label>
                          <input
                            type="file"
                            onChange={handleFileUpload}
                            disabled={status !== "idle"}
                            className="block w-full text-sm text-slate-medium file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:bg-crail-50 file:text-crail-600 hover:file:bg-crail-100"
                          />
                          {config.uploadedReference && (
                            <div className="mt-2 relative inline-block">
                              <img
                                src={config.uploadedReference}
                                className="h-20 w-20 object-cover rounded-lg border border-crail-300"
                                alt="Ref"
                              />
                              <button
                                onClick={() =>
                    ...
                    <div className="space-y-2">
                      <h3 className="font-serif text-lg font-semibold text-slate-dark mb-2">
                        Completed Sets
                      </h3>
                      {projectCompletedSets.length === 0 ? (
                        <p className="text-sm text-slate-400 italic">
                          No sets yet.
                        </p>
                      ) : (
                        projectCompletedSets.map((set) => (
                          <div
                            key={set.id}
                            className="bg-white rounded-lg border border-ivory-dark overflow-hidden"
                          >
                            <div
                              className="flex justify-between items-center p-3 bg-pampas cursor-pointer hover:bg-ivory-medium"
                              onClick={() => handleToggleCollapse(set.id)}
                            >
                              ...
                              <div className="p-3 bg-white border-t border-ivory-dark">
                                ...
                                <div className="grid grid-cols-4 gap-2">
                                  {set.items.slice(0, 8).map((item) => (
                                    <div
                                      key={item.id}
                                      className="aspect-square bg-slate-100 rounded overflow-hidden relative group cursor-pointer"
                                      onClick={() =>
                                        setViewerImage({
                                          src: item.imageUrl!,
                                          id: item.id,
                  ...
                  <div className="w-full lg:w-2/3 flex flex-col gap-4">
                      ...
                      <div className="bg-white rounded-xl shadow-card border border-crail-100 overflow-hidden flex flex-col max-h-[400px]">
                        <div className="p-4 bg-pampas border-b border-ivory-dark flex justify-between items-center">
                          <div className="flex items-center gap-3">
                            ...
                            ></div>
                            <div>
                              <h3 className="font-bold text-slate-dark text-sm">
                                {status === "paused"
                                  ? "PAUSED"
                                  : "PROCESSING BATCH"}
                              </h3>
                              <p className="text-xs text-slate-500">
                                ...
                                / {projectQueue.length} Completed
                              </p>
                            </div>
                          </div>
                          {nextItemCountdown !== null && (
                            <div className="flex items-center gap-2">
                              <div className="bg-slate-dark text-white px-3 py-1 rounded font-mono text-sm font-bold">
                                {nextItemCountdown}s
                              </div>
                              <button
                                onClick={handleSkipWait}
                                className="text-xs text-crail-500 underline hover:text-crail-700"
                              >
                                Skip
                              </button>
                            </div>
                    ...
                    {(sheetStatus.status !== "pending" ||
                      ...
                      <div className="bg-white p-4 rounded-xl shadow-card border border-ivory-dark flex gap-4">
                        <div className="w-32 flex-shrink-0">
                          ...
                          <div
                            className="aspect-[4/3] bg-slate-100 rounded overflow-hidden border border-ivory-dark relative cursor-pointer"
                            onClick={() =>
                              sheetStatus.url &&
                              setViewerImage({
                                src: sheetStatus.url,
                        ...
                        <div className="flex-grow">
                          <div className="flex justify-between items-center mb-1">
                            <label className="text-xs font-bold text-slate-dark">
                              Refinement Instructions (Affects Next Regen)
                            </label>
                            <button
                              onClick={handleRegenerateSheet}
                              disabled={status === "batch_running"}
                              className="text-[10px] font-bold text-crail-500 hover:underline disabled:opacity-50"
                            >
                              REGENERATE DNA
                            </button>
                          </div>
                          <div className="flex gap-2">
                            <input
                              className="flex-grow px-3 py-2 text-sm border border-ivory-dark rounded focus:border-crail-500"
                              placeholder="e.g. Make hair blonder, remove hat..."
                              value={sheetRefinement}
                              onChange={(e) =>
                                setSheetRefinement(e.target.value)
                              }
                            />
                          </div>
                          <p className="text-[10px] text-slate-400 mt-1">
                            Tip: Pause the batch to modify this and regenerate
                            the DNA sheet mid-set.
                          </p>
                        </div>
                      </div>
                    )}
                    ...
                    {status === "idle" || status === "batch_complete" ? (
                      <Card className="flex-grow flex flex-col">
                        <h3 className="font-serif text-lg font-semibold text-slate-dark mb-4">
                          Input Script
                        </h3>
                        <TextArea
                          placeholder={`SET 1: JUNGLE EXPEDITION\nContext: 1920s explorer...\n\nSCENE 1: Map room\nSCENE 2: Packing jeep`}
                          value={batchInput}
                            ...
                            >
                              <div className="sticky top-0 bg-pampas z-10 px-4 py-2 flex justify-between items-center border-b border-ivory-dark shadow-sm">
                                <div
                                  ...
                                  onClick={() => handleToggleCollapse(setName)}
                                >
                                  <span className="text-xs text-slate-500 font-mono">
                                    {isCollapsed ? "▶" : "▼"}
                                  </span>
                                  <span className="font-bold text-xs text-slate-dark uppercase">
                                    {setName}
                                  </span>
                                </div>
                                <div className="flex items-center gap-3">
                                  ...
                                  <button
                                    onClick={() => setEditingSet(setName)}
                                    className="text-[10px] text-crail-500 hover:underline"
                                  >
                                    Edit Settings
                                  </button>
                                </div>
                                    ...
                                    <div
                                      ...
                                      className={`flex gap-3 p-3 rounded-lg border transition-all ${
                                        item.status === "processing"
                                          ...
                                          : "bg-white border-ivory-medium"
                                      }`}
                                    >
                                      <div
                                        className="w-16 h-16 bg-white rounded border border-slate-100 flex-shrink-0 overflow-hidden cursor-pointer"
                                        onClick={() =>
                                          item.status === "success" &&
                                          setViewerImage({
                                            src: item.imageUrl!,
                                      ...
                                      <div className="flex-grow min-w-0">
                                        ...
                                        {item.status === "error" ? (
                                          ...
                                          <p className="text-xs text-slate-500 truncate mt-1 font-mono">
                                            {item.fullPrompt.substring(0, 60)}
                                            ...
                                          </p>
                                        )}
                                      </div>
